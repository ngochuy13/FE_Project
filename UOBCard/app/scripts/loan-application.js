...
jQuery(function($){
  ...
  window.CardRegistration = (function() {
    var module = {},
        ...
        validator,
        loanCardData = {
          'one-card': 'images/card-register/card-faces/one-card-face.png',
          'lady-card': 'images/card-register/card-faces/lady-card-face.png',
          'visa-sign-card': 'images/card-register/card-faces/visa-signature-face.png',
          'prvi-amex-card': 'images/card-register/card-faces/prvi-amex-face.png'
        },
          ...
          if(associatedField.filter(":visible").length){
            if(associatedField.get(0).tagName.toLowerCase() === 'input'){
              if(associatedField.attr('type') === "radio"){
                $(this).html( associatedField.filter(":checked").val() );
              }else{
    ...
    var processStep1ExCustomerData = function(){
      $("#existing-customer").each(function(){
        var fullName = $(this).find(".full-name input").val();
        var email = $(this).find(".email input").val();
        var mobile = $(this).find(".contact input").val();
        var title = $(this).find(".customer-title input:checked").val();
        view.find("[name='fullname']").val(fullName);
        view.find("[name='email']").val(email);
        view.find("[name='title']").val(title);
        view.find("[name='exCustomerContact']").val(mobile);
        view.find("[name='exCustomerNRIC']").val(title);
        view.get(0).submit();
      });
    };
    ...
    var submitStepInit = function(){
      ...
      view.find('.step-1-submission .btn-submit').on('click', function(e){
        ...
        if(view.valid()){
          ...
          if( view.find(".lead-form-wrapper").children("div:visible").attr('id') == "existing-customer" ){
            processStep1ExCustomerData();
            return;
          }
    ...
    module.init = function() {
      ...
      $("[name='fullnameOnUOBCard']").on('change', function(){
        $('.uob-card-name').html(this.value);
      });
      $(".applyUOBCard input").on('change', function(){
        var container = $(this).closest('.applyUOBCard');
        var selected = container.find('input:checked');
        if(selected.length){
          container.find('.card-face').attr('src', loanCardData[selected.attr('data-card')]);
        }else{
          var cardName = container.find('input').first().attr('checked','checked').attr('data-card');
          container.find('.card-face').attr('src', loanCardData[cardName]);
        }
      });
