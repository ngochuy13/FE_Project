...
var dependecyStyles = {
  'vendors.scss' : [
    ...
    'bower_components/ionrangeslider/css/ion.rangeSlider.skinHTML5.css'
  ]
};
...
gulp.task('dist-asset-to-server', function () {
  gulp.src(folders.dist+"/*.html").pipe(gulp.dest(folders.server));
  gulp.src(folders.dist+"/styles/*.css").pipe(gulp.dest(folders.server+"/styles"));
  gulp.src(folders.dist+"/scripts/*.js").pipe(gulp.dest(folders.server+"/scripts"));
  return gulp.src(folders.dist+"/images/*.*").pipe(gulp.dest(folders.server+"/images"));
});
